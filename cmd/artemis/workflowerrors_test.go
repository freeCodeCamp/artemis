...
func TestNewLiveAliasReader_RejectsFormatWithoutSiteToken(t *testing.T) {
	_, err := newLiveAliasReader(stubAliasGetter{}, "no-token/production")
	require.ErrorContains(t, err, "<site>")
}

func TestNewLiveAliasReader_PropagatesGetterError(t *testing.T) {
	read, err := newLiveAliasReader(stubAliasGetter{err: errors.New("r2 down")}, "<site>/production")
...
func TestNewLiveAliasReader_CollectsTargets(t *testing.T) {
	read, err := newLiveAliasReader(stubAliasGetter{val: "d1"}, "<site>/production", "<site>/preview")
	require.NoError(t, err)
