...
func runWith(rootCtx context.Context, cfg *config.Config) error {
	...
	if pgDB != nil {
		...
		slog.Info("gc.wired",
			...
			"reservationSweepReady", gcw.Reservations != nil && gcw.Lifecycle != nil,
			"siteReclaimReady", gcw.Reclaim.Claimer != nil && gcw.Reclaim.Releaser != nil,
...
func configureLogger(lvl slog.Level, extra slog.Handler) {
	h := observability.NewScrubbingHandler(
