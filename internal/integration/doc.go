...
//	              team must appear under the site's `teams:` list in
//	              sites.yaml). `gh auth token` is the easiest source on a
//	              dev laptop. CI can pass any PAT or a workflow token.
//
// Optional environment:
//
//	SITE          Target site key in sites.yaml. Default: `test`.
...
//     staff-only smoke target in `config/sites.yaml`. Real customer sites
//     are untouched.
