...
`/api/repo*` is mounted only when `RepoEnabled()` is true (Apollo-11 App credentials configured — see Configuration). `DELETE /api/site/{slug}?purge=true` also moves the site's R2 prefix to `_trash/` and records a tombstone; the same team gate applies as for the plain delete. A purge moves the two alias objects before the rest of the prefix, so the site stops serving within the 15s serve-cache TTL whatever its size, and it answers `502 r2_move_incomplete` rather than `200` if the source prefix still lists objects. A purge whose registry row is already absent is satisfied, not an error: it returns `200`, where the bare `DELETE` returns `404`. Every purge that reaches R2 writes one `audit_log` row — `outcome=success`, or `outcome=failure` with `stage` and `moved` naming what was already destroyed. A purge refused before that point, by the site lock or a missing tombstone store, changes nothing and writes no row. The bare `DELETE` only removes the registry row. `POST /api/site/{site}/deploys/{deployId}/restore` reverses a `DELETE .../deploys/{deployId}` tombstone. It moves the bytes back from `_trash/` and marks the deploy active again. `GET /api/site/{site}/trash` lists the site's tombstoned deploys with their purge-eligibility `expiresAt` (`CLEANUP_RECOVERY_DAYS` after `trashedAt`).
