...
[`docs/api/openapi.yaml`](api/openapi.yaml) is the authoritative surface (ADR-016, amendment 2026-09-03). `TestOpenAPI_NamesExactlyTheMountedRoutes` in `internal/server` keeps its routes and methods level with the router, so a route change that does not update the document fails CI. Request and response shapes are documented from the handlers and are not machine-checked; a shape change updates the document in the same commit. The table below is a reading aid:
...
Every routed response carries `X-Artemis-Version` (the build version, `dev` in local runs) and `X-Request-ID`. No request is rejected on the version.

`/readyz` probes the three upstreams concurrently (5 s each, `internal/handler/readyz.go`). No upstream failure returns `503`. An unreachable Valkey, R2 or Postgres returns `200 {"ready":true,"degraded":true}` and the pod stays in rotation, logging `readyz.valkey.degraded`, `readyz.r2.degraded` or `readyz.postgres.degraded` at Warn. Each upstream degrades rather than drains because every replica shares the same one, so a fault hits all of them at once and a `503` would empty the Service; the endpoints that need the upstream still answer their own error per request. A failing Valkey or R2 probe reaches Sentry only after `readyzPageThreshold` (3) consecutive failures, and then once per outage until the probe recovers. The Postgres branch logs but does not page. See `docs/design/0007-readyz-degradation.md` for the ruling. The R2 probe is one `ListObjectsV2` with retries disabled and its own 3 s HTTP ceiling (`internal/r2/r2.go`), so a probe failure measures R2, not the SDK's retry budget.

`/api/repo*` is mounted only when `RepoEnabled()` is true (Apollo-11 App credentials configured — see Configuration). `DELETE /api/site/{slug}` removes both R2 alias objects and then reserves the name for `SITE_RESERVATION_GRACE` (default 72h); see ADR 0006 and `docs/COMPATIBILITY.md` entries 19-20. The HTML and the assets stop serving within the 15s serve-cache TTL: the serve plane sends `Cache-Control: public, max-age=0, must-revalidate`, so the Cloudflare edge revalidates every request against the origin. `?purge` is retired and **refused**: any true-ish value answers `400 purge_retired` and performs no delete, because a `204` would tell a takedown or storage-reclaim caller the bytes were gone while they sat at the origin prefix. An explicit `?purge=false` is an ordinary delete. `POST /api/site/{slug}/undelete` returns a reserved name to service inside the grace window; past the deadline it answers `404`, because the reclaim sweep owns the row from that moment. `POST /api/site/{slug}/release` ends the reservation early and reclaims the bytes at once; it is gated on `REPO_APPROVE_AUTHZ_TEAM`, not `REGISTRY_AUTHZ_TEAM`, because delete is reversible and release is not. A `DELETE` on a name the serve plane answers but the registry does not know — an orphaned alias, the state `drift-detect` reports as `drift.orphan_aliases` — removes the aliases and answers `200 { slug, status: "unpublished", reserved: false }`. Nothing is reserved there, so `undelete` cannot bring it back. A name that nothing served and nothing registered still answers `404`. Every delete writes one `audit_log` row — `outcome=success`, or `outcome=failure` with `detail.stage` of `unpublish` or `reserve` naming how far it got. `POST /api/site/{site}/deploys/{deployId}/restore` reverses a `DELETE .../deploys/{deployId}` tombstone. It moves the bytes back from `_trash/` and marks the deploy active again. `GET /api/site/{site}/trash` lists the site's tombstoned deploys with their purge-eligibility `expiresAt` (`CLEANUP_RECOVERY_DAYS` after `trashedAt`).
...
The `drift-detect` cron sends an event as soon as it finds a single reclaimable deploy. That threshold was 25 while production carried a standing backlog of 36; the 2026-08-29 drain took the floor to zero and three consecutive nightlies confirmed it, so any reclaimable deploy is now an anomaly rather than residue. Reclaimable drift alerts a person and does not fail the run. A green check-in with no event therefore means "the job ran and found nothing to do", and a missed check-in is the only signal that the job stopped running. Both rows above are needed: the Crons row proves the job runs, and the drift row carries what it found.
