...
1. **`site-purge` writes a sentinel tombstone with `id = ''`**, and that row blocks reindexing of *every* deploy in that site until the recovery window clears it. That is deliberate and easy to mistake for a bug. No code path writes one any more — the purge branch is deleted and `?purge` is refused — but `POST /api/site/{slug}/release` writes the same sentinel through `RecordSiteTombstone`. The sentinel rows in production predate the release.
