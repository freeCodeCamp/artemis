...
| Only reclaimable drift     | Sentry event at `reclaimableAlertThreshold` (1 since the 2026-08-29 drain; 25 before), else log the counts | Corrected 2026-08-24. This row read "Log the counts. No event" and the code has never matched it: `cmd/artemis/driftalert.go:24,60` sends an event at the threshold. See the note below — the threshold is a known-wrong instrument, not a mismatch to be resolved by silencing the alert. |
