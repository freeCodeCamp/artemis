...
| ✓   | Observability — artemis is Sentry-only + independent (v1.4.0)                                   | **diverged (intentional)**              | Constellation apps use the platform stack; artemis manages that stack so it self-monitors via an EXTERNAL Sentry only (no GlitchTip / vmagent / `/metrics`). See ADR-016 + `docs/README.md`.                                                                                  |
| ✓   | Retiring Windmill **cleanup cron** (not Windmill itself)                                        | **aligned**                             | Windmill stays for platform-ops; only its `cleanup_old_deploys` flow is retired.                                                                                                                                                                                              |
